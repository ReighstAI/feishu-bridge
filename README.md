...
7. （推荐，能用之后再做）帮我设一个长期登录 token，免得以后登录态过期、桥突然
   全部不回还没提示。跑 `claude setup-token`，把它给的网址显示给我 → 我在浏览器
   登录、把页面上的 code 发回给你 → 你拿 code 换到 token 后，用 `umask 077` 创建
   `~/.claude/channels/lark/oauth-token` 把 token 写进去（权限 600，token 绝不要
   打印出来或写到别处），再 `launchctl kickstart -k gui/$(id -u)/com.feishu-bridge.daemon`
   重启。这个 token 走订阅计费、约一年有效，桥会自动优先用它。
...
- **本来好好的，突然全部不回了，日志（`tail ~/.claude/channels/lark/server.log` 或终端里的桥会话）出现 `Please run /login · API Error: 401`**：keychain 里的登录态（OAuth token）到期了、自动续期没成，得重新登录一次（终端 `claude` → `/login`）。想一劳永逸、不再被这个打断，换成长期 token：
  ```bash
  claude setup-token                    # 浏览器登录，复制它打印的 token
  umask 077
  printf '%s' '把token粘到这里' > ~/.claude/channels/lark/oauth-token
  launchctl kickstart -k gui/$(id -u)/com.feishu-bridge.daemon   # 重启，桥即改用它
  ```
  这个 token 走订阅计费、约一年有效，桥会自动优先用它，过期掉线基本消失。
